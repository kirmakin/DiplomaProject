...
public class OrderPage {
    ...
    private final String expireDateText = "Истёк срок действия карты";
    private final String incorrectMonthYearText = "Неверно указан срок действия карты";
    ...
    public void shouldBeExpiredDateError() {
        $(withText(expireDateText)).
                waitUntil(visible, 15000);
    }

    public void shouldBeIncorrectMonthYearError() {
        $(withText(incorrectMonthYearText)).
                waitUntil(visible, 15000);
    }
    ...
    public void shouldBeIncorrectCvcErrors() {
        List<SelenideElement> errorMessagesBox = $$(fieldErrorSelector);
        errorMessagesBox.get(4).$(withText(incorrectFormatErrorText)).waitUntil(visible, 15000);
    }
}
